...
### Changed ###
* (refactor) `ArticleEvent::fetchPostImages()` now resolves images through the new `resolveContentImageIds()`, which reads block attributes (`core/image`, `core/cover`, `core/media-text`, legacy `core/gallery`) via `parse_blocks()`, `wp-image-<id>` classes the way core does in `wp_filter_content_tags()`, and — only for `<img>` tags carrying neither — upload URLs via `attachment_url_to_postid()`. IDs are deduplicated, the featured image is excluded (it is dispatched separately as the hero), and IDs that no longer resolve to an image attachment are skipped. `isImageAttachedAndStillUsed()` and the `get_attached_media()` call have been removed.
...
> **Note for implementers:** this fix restores images that were previously being dropped, so articles will legitimately start dispatching images they have never sent before. Measured over 2452 published articles on one property: 78 stale entries removed across 59 articles, and 2569 genuinely-in-body images restored across 814 articles. Plan a bulk re-sync accordingly.
