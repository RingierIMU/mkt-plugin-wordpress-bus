...
### Changed ###
* (refactor) `ArticleEvent::fetchPostImages()` now resolves images through the new `resolveContentImageIds()`, which reads block attributes (`core/image`, `core/cover`, `core/media-text`, legacy `core/gallery`) via `parse_blocks()`, `wp-image-<id>` classes the way core does in `wp_filter_content_tags()`, and — only for `<img>` tags carrying neither — upload URLs via `attachment_url_to_postid()`. IDs are deduplicated in order of appearance, the featured image is excluded (it is dispatched separately as the hero), and IDs that no longer resolve to an image attachment are skipped. `isImageAttachedAndStillUsed()` and the `get_attached_media()` call have been removed.
* (behaviour) `images[]` entries are now ordered by appearance in the article body.
