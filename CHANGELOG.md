...
### Fixed ###
* (bug) Stale and missing images in the `images[]` array of `ArticleCreated` / `ArticleUpdated` payloads. `fetchPostImages()` enumerated images with `get_attached_media()`, i.e. by `post_parent` (ownership), and WordPress never resets `post_parent` when an editor removes an image from an article ([core #30691](https://core.trac.wordpress.org/ticket/30691#comment:12)). The only safeguard was `isImageAttachedAndStillUsed()`, an unanchored `str_contains()` of the attachment slug against the raw content — which failed in **both** directions, because WordPress appends a collision suffix to the slug and to the filename independently. False positive: a removed image slugged `uzucapiune` is a substring of its replacement `uzucapiunea`, so the dead image was dispatched on every event. False negative: a live image slugged `bloc-2` is stored as `bloc.jpg`, so its slug appears nowhere in the content and it was never dispatched at all. A single article could hit both — sending the image the editor had removed while omitting the one that replaced it. Images are now resolved by attachment ID from the article body itself, never from the attachment relationship.
* (bug) Markup an editor had commented out still contributed images. HTML comments are now stripped before the content is scanned, except the `<!-- wp:… -->` block delimiters and the `<!--more-->` / `<!--nextpage-->` markers.
* (bug) A protocol-relative `<img src="//host/…">` resolved to nothing — `attachment_url_to_postid()` cannot read one. URLs are now matched on the path below `wp-content/uploads/` and rebuilt against this site's own upload base, so the scheme and host they carry no longer matter.
* (bug) The `wp-image-<id>` match was unanchored, so a longer class such as `not-a-wp-image-12` was read as attachment 12. The class name is now anchored.
* (bug) A `wp-image-<id>` class left behind by a content migration was believed over the `<img src>` it sits on, so an article could dispatch photographs belonging to a different article. Content migrated between properties keeps the *source* site's attachment IDs, which resolve locally to unrelated images. Resolution now runs URL-first: the attachment holding the URL's upload path wins over the class, the class is consulted only when nothing holds that path (an image hosted elsewhere, or moved) and is then checked by file name, and a URL that is not one of our uploads resolves to nothing rather than to the wrong picture. Comparing file names is not sufficient on its own — two uploads of the same name in different month folders compare equal, and WordPress appends its `-1` dedup suffix *after* the size, so `x-300x211.jpg` and `x-300x2111.jpg` are two different pictures sharing one stem. (Core's `wp_filter_content_tags()` reads the same class, but only to decorate the tag it found it on — it never substitutes the URL, which is why its precedent does not extend this far.)
* (bug) A rejected ID was barred across the whole article, so one stale class copied onto several tags — the usual shape of this corruption — also killed the tag it was right for. A rejection now clears only where a URL positively confirmed that ID.
* (bug) Block attributes reached the payload with no reconciliation at all. A block that renders no `<img>` — a `core/cover` drawing its image as a CSS background — bypassed every check, leaving the original defect unmitigated on any property using such blocks. (A legacy `core/gallery` carries bare IDs with no URL beside them, so those are still taken on trust; they are at least now restricted to that block.) Block IDs are now reconciled against the block's own `url` / `mediaUrl`.
* (bug) No sub-size of a `-scaled` upload could be resolved. WordPress stores such an upload as `name-scaled.ext` but names its sub-sizes, and the retained original, off the unscaled stem — 363 of 6738 upload URLs in one corpus resolved to nothing and fell back to name matching. `-scaled` is now a resolution candidate.
* (bug) An `<img>` whose `src` was unquoted, or carried no space before the attribute, parsed as having no `src` at all — and the code then trusted the class unchecked. `src` is now parsed with a tolerant matcher.
* (bug) Non-ASCII filenames were corrupted during resolution, so their images were discarded. `parse_url()` replaces bytes in the C1 range (0x80-0x9F) with an underscore, and those are the UTF-8 continuation bytes of `ă`, `ș`, `ț` and `×` — the path then matched no attachment and the `wp-image-<id>` class was rejected rather than merely unresolved. Path extraction is now byte-safe, and the URL as written is tried before any rebuilt spelling.
* (bug) `core/media-text` reconciliation could only ever reject: `mediaUrl` is `source: attribute` so `parse_blocks()` never yields it, and `mediaLink` holds the attachment permalink rather than a file URL. `mediaLink` is no longer consulted.
* (bug) A `data:` placeholder `src` — used by lazy-loading and imported markup — was read as contradicting the class and barred that ID across the article. A non-http scheme now abstains.
* (bug) Tags were found with a regex, which cannot parse HTML: `<img[^>]*>` stops at the first `>` even inside a quoted value, so an ordinary `alt="preț > 100.000 EUR"` split the tag — losing the `src` and leaving the class to be accepted unchecked, or, with the attributes the other way round, dispatching one of our images for an article whose only image is on another server. Tags are now read with `WP_HTML_Tag_Processor` (WordPress 6.2+; the regex remains as a fallback below that), which also scopes `wp-image-<id>` to the class attribute and stops a `src=` written inside an `alt` value from outranking the real one.
* (bug) Percent-encoded filenames were rejected rather than merely unresolved, since nothing decoded them. Both sides are now compared decoded as well as raw.
* (bug) A file-name match — which ignores the folder and the host — could cancel a rejection made by resolving an exact upload path. It still accepts the ID, but no longer clears a rejection.
* (bug) `id`, `mediaId` and `ids` were read from *any* block, so `core/video` and `core/file` reached the payload on the mime check alone and a third-party block with an `ids` array could contribute anything. Restricted to `core/image`, `core/cover`, `core/media-text` and `core/gallery`.
* (bug) Blocks were parsed from the unstripped content, so a block the editor had commented out still contributed its `id`.
* (behaviour) Lazy-loaded markup resolves: where `src` holds a placeholder, `data-src` is read, so the class is checked rather than taken on trust.
* (behaviour) An image edited in WordPress (`name-e<timestamp>.ext`) resolves from the pre-edit name the body still carries. Looked up last, after every rewritten spelling has missed, because it costs a `LIKE` over `postmeta`.
* (bug) The HTML-comment keep-list matched on prefix, so `<!-- moreover … -->` survived stripping as though it were a `<!--more-->` marker. It is now word-anchored.
* (bug) Images were resolved from `get_the_content()`, which returns the teaser only for a body carrying `<!--more-->`, page one only for `<!--nextpage-->`, and the password form for a protected post — silently dropping every image below the cut. Resolution now reads `$post->post_content`. The `body` payload field is unchanged.

### Changed ###
* (refactor) `ArticleEvent::fetchPostImages()` now resolves images through the new `resolveContentImageIds()`. It reads block attributes (`core/image`, `core/cover`, `core/media-text`, legacy `core/gallery`) via `parse_blocks()` and `wp-image-<id>` classes, reconciles each attachment ID against the `src` of its own `<img>` tag, and falls back to resolving that URL against the media library. IDs are deduplicated, the featured image is excluded (it is dispatched separately as the hero), and IDs that no longer resolve to an image attachment are skipped. `isImageAttachedAndStillUsed()` and the `get_attached_media()` call have been removed.


### Added ###
* (debug) Opt-in payload capture. With `define('RINGIER_BUS_DEBUG_PAYLOAD', true);` in `wp-config.php`, every article dispatch writes the exact JSON being POSTed to `wp-content/buslog/payload-<created|updated|deleted>-<post_id>.json`, overwritten each time so each file holds the latest payload of its kind. The event type is in the name because publishing dispatches twice — instantly as created, then a minute later as updated — and one file per article would let the second quietly replace the first. Written *before* the request goes out, so the payload is captured even when the BUS does not respond. Pretty-printed with unescaped unicode and valid JSON, so it pipes straight to `jq`. Off entirely without the constant. The directory is protected by `index.php` and `.htaccess`; on nginx add `location ~* /wp-content/buslog/ { deny all; }` to the server config.
...
### Performance ###
* (perf) An image's `content_hash` is computed once and stored against the attachment, instead of on every event. An image that cannot be read is logged against its article — ID and slug — and dispatched with an empty `content_hash`, which the contract permits; the next event for that article tries again. Where media is offloaded to S3 or a CDN there is no local file, so `Utils::hashImage()` downloads it over HTTP inline in the dispatch — previously for every image of every article, every time. The stored hash is fingerprinted on file size and mtime locally, and on the stored path plus the attachment's modified time when offloaded, so a replaced image is re-hashed. The fetch timeout drops from 10s to 5s.
