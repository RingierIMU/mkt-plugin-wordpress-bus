...
* (bug) A `wp-image-<id>` class left behind by a content migration was believed over the `<img src>` it sits on, so an article could dispatch photographs belonging to a different article. Content migrated between properties keeps the *source* site's attachment IDs, which resolve locally to unrelated images. Each ID is now reconciled against the `src` of its own tag: an attachment holding the URL's exact upload path — folder included — outranks the class, the class is accepted when it names that same path, the file name alone is consulted only when nothing holds the path (an image hosted elsewhere, or moved), and a URL that is not one of our uploads resolves to nothing rather than to the wrong picture. Comparing file names alone is not enough: two uploads of the same name in different month folders compare equal, which let a stale ID through. Core's `wp_filter_content_tags()` only uses the ID to decorate the tag it found it on — this payload substitutes the URL, so the ID has to be shown to describe that `src` first.
...
### Added ###
* (filter) `ringier_bus_article_image_ids` — `(int[] $image_id_list, int $post_ID, string $content): int[]`. Adjust the non-hero attachment IDs an article dispatches, just before they are turned into payload entries. See the readme.

> **Note for implementers:** this fix restores images that were previously being dropped, so articles will legitimately start dispatching images they have never sent before. Measured over 2452 published articles on one property: 2544 images restored across 797 articles, and 78 entries removed across 60 articles (every removal audited — none is an image the article still shows). Plan a bulk re-sync accordingly.
