...
* (bug) A `wp-image-<id>` class left behind by a content migration was believed over the `<img src>` it sits on, so an article could dispatch photographs belonging to a different article. Content migrated between properties keeps the *source* site's attachment IDs, which resolve locally to unrelated images. Resolution now runs URL-first: the attachment holding the URL's upload path wins over the class (except where the URL is a size generated for the attachment the class names — see below), the class is consulted only when nothing holds that path (an image hosted elsewhere, or moved) and is then checked by file name, and a URL that is not one of our uploads resolves to nothing rather than to the wrong picture. Comparing file names is not sufficient on its own — two uploads of the same name in different month folders compare equal, and WordPress appends its `-1` dedup suffix *after* the size, so `x-300x211.jpg` and `x-300x2111.jpg` are two different pictures sharing one stem.
...
* (bug) Block attributes reached the payload with no reconciliation at all. A block that renders no `<img>` — a `core/cover` drawing its image as a CSS background, a legacy `core/gallery` — bypassed every check, leaving the original defect unmitigated on any property using those blocks. Block IDs are now reconciled against the block's own `url` / `mediaLink`.
...
* (bug) Where a URL is a rendition of the attachment the class names, the class wins; a library can hold both `photo.jpg` and a separate upload named `photo-224x300.jpg`.
* (bug) The HTML-comment keep-list matched on prefix, so `<!-- moreover … -->` survived stripping as though it were a `<!--more-->` marker. It is now word-anchored. Core's `wp_filter_content_tags()` only uses the ID to decorate the tag it found it on — this payload substitutes the URL, so the ID has to be shown to describe that `src` first.
