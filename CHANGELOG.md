...
> **Note for implementers:** this fix restores images that were previously being dropped, so articles will legitimately start dispatching images they have never sent before. Measured over 2452 published articles on one property: 2545 images restored across 798 articles (each attachment becomes 4 payload rows, so roughly 10,000 additional rows), and 78 removed across 60 articles. Every removal was audited by decoded-pixel comparison: 29 are pictures the article no longer shows, and the other 49 are duplicate re-uploads whose picture is still dispatched under the attachment the body actually references — no information is lost. Plan a bulk re-sync accordingly.
